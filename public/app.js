...
async function saveProduct(e){
  ...
  try{
    await req('/products', {
      method:'POST',
      body: JSON.stringify({
        seller_id:c.seller_id, seller_token:c.seller_token,
        id:$('productId').value || undefined,
        name:$('productName').value, price:$('productPrice').value, currency:$('productCurrency').value || 'MRU',
        category:$('productCategory').value, image_url:$('productImage').value, description:$('productDesc').value,
        status:$('productStatus').value
      })
    });
    e.target.reset(); $('productId').value='';
    toast('تم حفظ المنتج');
    loadSellerProducts();
  }catch(e){toast(e.message)}
}
